...
--Listar información básica de las oficinas: Recupera el código, ciudad, país y teléfono de todas las oficinas registradas.
select codigo_oficina, ciudad, pais, telefono
from oficina;
--Obtener los empleados por oficina: Muestra el nombre, apellidos y puesto de los empleados que trabajan en cada oficina, agrupados por código de oficina.
select e.nombre, e.apellido1, e.apellido2, e.puesto, e.codigo_oficina
from empleado e
join oficina o on e.codigo_oficina = o.codigo_oficina
order by e.codigo_oficina;
--Calcular el promedio de salario (límite de crédito) de los clientes por región: Determina el promedio del límite de crédito de los clientes agrupados por región.
...
--Obtener productos disponibles y en stock: Lista el código, nombre y cantidad en stock de todos los productos que tienen al menos 1 unidad disponible.
...
--Pedidos pendientes por cliente: Lista el código y estado de todos los pedidos que no han sido entregados (estado diferente de "Entregado") junto con el nombre del cliente.
select p.codigo_pedido, p.estado, c.nombre_cliente
from pedido p
join cliente c on p.codigo_cliente = c.codigo_cliente
where p.estado != 'Entregado';
--Total de productos por categoría (gama): Obtén la cantidad total de productos agrupados por categoría (gama).
...
--Pedidos realizados en un rango de fechas: Recupera los códigos de pedido y las fechas de los pedidos realizados entre dos fechas específicas.
select codigo_pedido, fecha_pedido
from pedido
where fecha_pedido between '2024-01-01' and '2024-12-31';
--Detalles de un pedido específico: Muestra el código de pedido, los productos involucrados, las cantidades y el precio total de cada línea de pedido.
...
--Productos más vendidos: Lista los productos con mayor cantidad vendida, ordenados de forma descendente por la cantidad total.
...
--Número total de empleados por oficina: Calcula la cantidad total de empleados asignados a cada oficina.
...
--Pagos realizados en una forma específica: Recupera los pagos realizados con una forma de pago específica, como "Tarjeta de Crédito".
...
order by mes;
--Clientes con múltiples pedidos: Muestra los nombres de los clientes que tienen más de un pedido registrado.
...
--Promedio, máximo y mínimo del límite de crédito de los clientes por país: Obtén el promedio, el valor máximo y el mínimo del límite de crédito agrupados por país.
select pais,
...
from cliente
group by pais;
--Historial de transacciones de un cliente: Recupera el historial de pagos realizados por un cliente específico, mostrando fecha, total y forma de pago.
select fecha_pago, total, forma_pago
from pago
where codigo_cliente = [codigo_cliente_especifico];
--Empleados sin jefe directo asignado: Muestra los empleados que no tienen asignado un código de jefe.
select nombre, apellido1, apellido2
from empleado
where codigo_jefe is null;
--Productos cuyo precio supera el promedio de su categoría (gama): Lista los productos cuyo precio de venta es mayor que el promedio de su gama.
...
--Promedio de días de entrega por estado: Calcula el promedio de días entre la fecha de pedido y la fecha de entrega agrupados por estado del pedido.
select estado, 
       avg(datediff(fecha_entrega, fecha_pedido)) as promedio_dias_entrega
from pedido
where fecha_entrega is not null
group by estado;
--Clientes por país con más de un pedido: Lista los países con la cantidad de clientes que tienen más de un pedido registrado.
